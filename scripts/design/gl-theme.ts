...
.gl-moves li[data-move]{display:grid;
  grid-template-columns:minmax(190px,auto) minmax(84px,auto) minmax(130px,1fr) minmax(130px,1fr);
  gap:10px 22px;align-items:start;
  /*
   * No rule under the row. The base .gl-moves style draws one, and against the rounded hover box
   * these rows now have it read as a stray outline around the first row only — the second row is
   * :last-child and loses its border, so one row looked boxed and the other did not. The hover
   * background is the affordance; a divider as well is two devices doing one job.
   */
  border-bottom:0}

/*
 * The transition chips are a fixed three-column grid, not a flex row.
 *
 * As flex, each chip sized to its own label, so "AMBER → RED" and "GREEN → AMBER" put the arrow and
 * the second chip at different x positions — two rows of the same construct that visibly did not
 * line up. The columns are fixed to the widest label so the arrow lands in the same place on every
 * row, whatever the bands happen to be.
 */
.gl-movepair{display:grid;grid-template-columns:84px 14px 84px;align-items:center;gap:8px;
  white-space:nowrap}
.gl-movepair .gl-rag{justify-content:center}
.gl-movepair .to{text-align:center}
