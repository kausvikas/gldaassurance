...
(function () {
  ...
  function session(base, need) {
    ...
    /*
     * Ask the deployment whether it will issue a session without a code, before asking a person.
     *
     * There is no client-side flag for "is this deployment open" — the client cannot know, and a
     * flag baked into the bundle would be a second source of truth that drifts from the server's.
     * So it simply asks: a deployment that answers with a token is open, one that answers 401 is
     * not, and only then is a person interrupted.
     *
     * \`need\` is 'full' where the surface needs to write, so Data Sources prompts immediately
     * rather than accepting a read-only session and failing at the receipt.
     */
    if (need === 'full') return prompt_(base);
    return fetch(base + '/session', {
      ...
      return r.json().then(function (payload) {
        var token = payload && payload.data ? payload.data.token : null;
